...
Hooks in `.claude/settings.json` (all fail-open, code in `.claude/hooks/`): **PreToolUse** = security/git guard (blocks dangerous commands, `.env`, bad commit format); **SessionStart** = injects the active project's brain (STATE head + next ticket — no blind start); **PostToolUse** = nudges to `sofi checkpoint` when the tree drifts uncommitted; **Stop** = logs a session breadcrumb to `.claude/memory/sessions.jsonl`.
