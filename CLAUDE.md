...
## Company brain
Live state per project: `projects/<PRJ-ID>/_context/{STATE,CONTEXT,DECISIONS,HANDOFFS,REGISTRY}.md`. Isolated by `PRJ-ID`. **Read/execute split — kills context amnesia (`engine/protocols/04-coordination-registry.md`):** the *brain layer* (CEO + the 5 tier-advisor masks the main session wears) reads the brain, decides, and delegates; *leaf specialists* get a distilled frozen slice inside the RCCF and go straight to execution — they never open the (154 KB+) brain. `REGISTRY.md` = the 1-line-per-artifact index the brain layer scans (`python3 engine/tooling/agents/ceo/registry.py add|list|find|verify`) to know "what exists" and to **verify deliverables without reading them** (exists·bytes·words·sha). 
