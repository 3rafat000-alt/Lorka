...
> end-to-end as the Lead Architect + UX Principal would. **Read-only. Never writes.**
...
```
[SOFI SYSTEM PROMPT]
Act as the Lead Solution Architect & UX Principal. Inspect the feature <FEATURE_NAME>
across all layers. Do NOT write code (Read-only static sweep).
...
- **Phase 1 — scan & draft (🟢 Haiku 4.5 / 🔵 Sonnet 5):** run the Python scanners (0 model
  tokens), grep static sweep across folders, gather flagged files into memory, and draft the
  raw SEV skeleton. Cheap models only — no judgment calls here.
- **Phase 2 — hard gate (🔮 Fable 5, gatekeeper):** hand the ENTIRE gathered context to
  Fable 5. It matches the 7 steel rules, checks Tier-A vulnerabilities, issues the final
  SEV report, and grants/denies code approval. Fable 5 is opened ONLY for this cross-layer
  gate — never for the mechanical scan. Opus 4.8 stays out entirely (last-resort debugging only).
  Route map: `spec-review-scan` / `spec-review-gate` in `engine/routing/routing.yaml`.
...
2. **Locate + steel-rule scan — Python tools (token-frugal, 0 model tokens):**
   ```bash
   python3 engine/tooling/agents/ceo/feature_scan.py "<FEATURE_NAME>" --prj <PRJ> --md
   python3 engine/tooling/agents/ceo/sofi_automator.py <project_dir>   # 7-steel-rules scanner (--rule N to isolate)
   ```
...
4. **Output** — the 4-pillar matrix. Each finding: `SEV · file:line · defect → fix`. Severity 🔴 breaks/security · 🟠 correctness · 🟡 quality/taste. **Normal prose, never compressed.**
5. **Verdict** — per pillar: sound / at-risk / broken, one line each. Then the single biggest risk.
6. **External review (standing, before handoff)** — push the SEV verdict through the desk: `sofi gemini review --prj <PRJ> --json --text "<the 4-pillar findings + context + ask for detailed remediation guidance>"` (inline, never a `.md`). Then ANALYZE + EXECUTE the reply autonomously and loop till the feature converges — don't stop to ask. `engine/protocols/external-review-desk.md`.
...
Full protocol: `engine/protocols/spec-review.md`.
