...
| 7-gap | **lesson-vaccine loop is broken** (found Phase-5): UserPromptSubmit hook calls `lessons_cache.vaccine_for` to inject per-prompt lessons, but the cache write-half (`add_lesson`) has zero callers → cache is always empty; `reflection_engine` writes lessons to LESSONS.md via `tickets.append_lesson`, a DIFFERENT store. Two disconnected lesson systems. | 🟡 | ⏳ Phase 7 — wire reflection → cache (or point `vaccine_for` at LESSONS.md) so injected lessons actually exist. |
