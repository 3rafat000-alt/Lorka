...
| 8 | ☁️ Cloud layer (Cloudflare + GitHub) | build | 🔶 PARTIAL — **GitHub CI ✅ live-green** (ca92820, pushed, runs on every push: YAML·imports·105-parity·secret-scan·junk). **Cloudflare dashboard ✅** (`dashboard/start.sh tunnel` via cloudflared, e8aeed4). **oracle ✅ hardened** — sanitizer now redacts GitHub/OpenAI/Slack tokens (closed the exact gap our PAT incident exposed) + fixed the dead last-ditch scrub (2062a62). Remaining: remote-agents = a Claude-Code harness capability, not framework code (needs concrete scope from owner). Persistent Cloudflare named-tunnel/custom-domain needs owner's CF account token. |
| 9 | Cutover + tag v7.0 | 🟡 | 🔶 CLAUDE.md header → v7 + v7-rebuild note. **Tag `v7.0` cut at this milestone.** Full CLAUDE.md/ORG.md prose regen + physical removal of v6 files deferred (architecture unchanged — same 15 rooms / 105 agents; only tooling PATHS moved, already sed-updated everywhere). |
