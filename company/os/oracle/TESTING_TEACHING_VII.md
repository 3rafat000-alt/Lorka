...
```bash
python3 company/os/agents/ceo/agent_preflight.py
...
cat << 'EOF' | python3 company/os/agents/ceo/agent_output_guard.py PRJ-SAKK
...
agent_output=$(python3 company/os/agents/ceo/agent_preflight.py)
echo "$agent_output" | python3 company/os/agents/ceo/agent_output_guard.py PRJ-SAKK
...
2. Add to SessionStart hook:
   ```bash
   python3 company/os/agents/ceo/agent_preflight.py > /dev/null
   ```
3. Wrap agent dispatch:
   ```bash
   company/os/agents/ceo/agent_wrapper.sh PRJ-SAKK <agent_command>
   ```
