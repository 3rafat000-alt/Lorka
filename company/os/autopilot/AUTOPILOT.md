...
Every division is backed by executable Python under `company/os/` — invoke these, don't hand-simulate:
- **Dispatcher:** `company/os/bin/sofi` (`sync · checkpoint · brain · route · gate-check · dispatch · squad · handoff · domain · tunnel · doctor`).
- **Static Engine:** `company/os/agents/ceo/sofi_scan.py` — unified AST/taint/taste scan (grep-first, token-frugal).
...
- **OODA Loop:** `company/os/ooda/engine/main.py` (v2 live). Model IDs pinned to `routing.yaml`.
...
- **Design-Taste Alignment:** `/sofi-design-taste` locks tokens; WCAG 2.2 AA overrides any variance dial automatically. The Design room lead drives the exit-gated console: `company/os/agents/uiux/uiux_pipeline.py` (`scan·brief·gate·rtl`) — spec `company/rooms/03-design/agents/dsn-lead.md`, spawnable `dsn-lead`.
