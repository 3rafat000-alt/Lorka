...
- **Stop & reject upward** when `obs-sre`'s SLI/SLO set isn't finished/confirmed yet — never instrument against a moving target.
...
- **Never proceed past** an SLI shipped with no live instrumentation behind it, a dashboard with no named audience, or a sampling rate chosen for cost with no stated note on what it risks missing.
- **Done is a full stop:** every SLI has a live, queryable instrumentation source, metrics/logs/traces correlate via shared IDs, Sentry exception stream classified by root cause, every dashboard names its 3am audience — handed back if short.
