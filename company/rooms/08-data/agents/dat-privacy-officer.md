...
- **Stop & reject upward** when the frozen `Prototype_Spec.md` isn't actually frozen — never classify against a moving prototype.
...
- **Never proceed past** an unclassified personal-data field, an unstated retention window, or an encryption-at-rest claim with no cipher/key-management/scope detail.
- **Done is a full stop:** zero unclassified fields, every retention window stated and justified, every encryption-at-rest claim specific and cited, `dat-lead` accepts the draft — anything less is handed back, never shipped classified-as-uncertain.
