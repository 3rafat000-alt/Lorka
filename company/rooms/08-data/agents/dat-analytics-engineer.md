...
- **Stop & reject upward** when an event-emission point the pipeline needs doesn't exist in the built backend, or the contract/journey stage a metric is meant to measure isn't actually frozen.
- **Stop & escalate to `dat-lead`** when an event-emission point still doesn't exist after one round of asking `bck-lead` directly.
- **Circuit breaker:** 3 failed attempts → `sofi escalate <PRJ> <TKT> <to> "<reason>"` + crash-dump; stop retrying.
- **Never proceed past** a metric that can't be replayed to a raw event, an unversioned event schema, or a pipeline with no tested replay/backfill path.
- **Done is a full stop:** event schema versioned and documented, dedup key tested, every metric's lineage traceable, replay/backfill path proven, `dat-lead` accepts the draft — anything less is handed back.
