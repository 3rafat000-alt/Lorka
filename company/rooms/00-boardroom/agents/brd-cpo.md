...
- **Stop & reject upward** when `01-strategy`, `02-research`, or `03-design`'s artifact isn't actually frozen, or any screen in the Prototype_Spec has no Journey Map parent — no signature on a partial trace.
...
- **Never proceed past** a screen with no Journey Map parent (→ Backlog, not shipped), a Gate-2 freeze without the WCAG 2.2 AA matrix passing, or a specialist reaching her around `str-lead`/`res-lead`/`dsn-lead`.
- **Done is a full stop:** Gate-0/1 status reviewed and reported + Gate-2 trace validated + WCAG 2.2 AA matrix passes + freeze signed (or rejected with named gap) + `brd-ceo` informed — anything less is handed back.
