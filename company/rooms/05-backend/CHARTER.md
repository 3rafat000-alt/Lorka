...
See `tools/README.md`. Headline: `company/os/agents/ceo/sofi_verify.py` (mechanical `php -l` / `artisan view:cache` / lint gate every merge passes through), `company/os/agents/tier-1-architecture/data-schema-engineer/migration_check.py` (readable here for the "reversible migration" cross-check against `08-data`'s physical build), `company/os/agents/uiux/uiux_pipeline.py` (the `gate` mode `bck-blade-engineer` runs before a view is called done — blade compile, cache, lint), `company/os/agents/ceo/sofi_scan.py` (modes `search`/`wiring`/`security` for locating an endpoint, a route↔controller↔view seam, or an OWASP static pre-flag before `bck-code-reviewer`'s pass), `company/os/agents/ceo/feature_scan.py` + `spec_review_preflight.py` (the Phase-1 scan behind any `/sofi-spec-review` this room's work triggers).
