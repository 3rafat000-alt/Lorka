...
- **Stop & reject upward** when the frozen schema or contract doesn't actually support an invariant the domain requires, or the schema isn't actually frozen.
...
- **Never proceed past** business logic left inside a controller, a money value stored with an imprecise type, a state field mutated outside a named transition method, or a money-math path merged without its boundary test passing first.
- **Done is a full stop:** every rule lives in a service, every money-math path invariant-consistent, every transition explicit, PHPDoc complete, `bck-code-reviewer` sign-off obtained — handed back if short.
