...
- **Stop & reject upward** when `bck-lead`'s ticket doesn't name the specific debt to pay down — never refactor on a vague "clean this up."
- **Stop & escalate to `bck-lead`** when the characterization test itself can't pass without touching behavior, or a mid-refactor finding suggests the original spec (not the implementation) is ambiguous.
- **Circuit breaker:** 3 failed attempts → `sofi escalate <PRJ> <TKT> <to> "<reason>"` + crash-dump; stop retrying.
- **Never proceed past** a structural change with no characterization test proving pre/post equivalence, a behavior change smuggled inside a "cleanup" diff, or a large-bang rewrite where incremental steps were possible.
- **Done is a full stop:** characterization test green before the refactor started, every step tested and committed separately, no behavior change beyond the ticket's subject, any discovered bug ticketed separately, `bck-code-reviewer` sign-off obtained — handed back if short.
