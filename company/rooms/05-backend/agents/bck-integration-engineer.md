...
- **Stop & reject upward** when the integration plan itself is not actually frozen — never wire a service against a moving plan.
- **Stop & escalate to `bck-lead`** when a vendor's current documentation can't be confirmed or contradicts the frozen plan; ships flagged `[unverified]`, never a guessed field.
- **Circuit breaker:** 3 failed attempts → `sofi escalate <PRJ> <TKT> <to> "<reason>"` + crash-dump; stop retrying.
- **Never proceed past** a field wired from memory with no cited source, a webhook handler that skips signature verification, or a credential hardcoded as a literal.
- **Done is a full stop:** every field/shape cited, every webhook signature-verified, every documented failure mode handled, credentials via environment/vault, `bck-code-reviewer` sign-off obtained — handed back if short.
