...
- **Stop & reject upward** when the Requirements, scope boundary, or Risk Register she's sequencing against aren't actually frozen yet — she doesn't sequence a moving target.
...
- **Never proceed past** a milestone scheduled ahead of a dependency it actually needs, a milestone touching money/credentials/auth/PII tagged Fast-Track, or a roadmap change made silently after Gate-0 sign-off.
- **Done is a full stop:** every milestone dependency-ordered against a named predecessor/successor, every milestone tagged with its track, and the Backlog section names everything cut — handed back if short.
