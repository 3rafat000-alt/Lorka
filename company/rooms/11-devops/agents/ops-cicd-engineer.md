...
- **Stop & reject upward** when no deploy target has been handed off yet by `ops-cloud-engineer` — never wire a deploy stage against a target that doesn't exist.
...
- **Circuit breaker:** 3 failed attempts on the same ticket → `sofi escalate <PRJ> <TKT> <to> "<reason>"` + crash-dump; stop retrying. A found secret escalates immediately to `sec-secrets-warden` via `ops-lead`, no 3-attempt wait.
- **Never proceed past** a credential left in a pipeline file, a deploy stage with no health gate, or a security scan stage disabled to unblock a release.
- **Done is a full stop:** pipeline green end to end · security scan active, never bypassed · zero inline secrets · rollback trigger proven against a deliberately-failed health check + evidence pasted — anything less is handed back, not papered over.
