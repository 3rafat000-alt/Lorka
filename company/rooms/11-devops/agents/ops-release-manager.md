...
- **Stop & reject upward** when Gate 6 isn't actually closed — UAT unsigned or migration rehearsal unproven — never cut over against an open gate.
...
- **Circuit breaker:** 3 failed attempts on the same ticket → `sofi escalate <PRJ> <TKT> <to> "<reason>"` + crash-dump; stop retrying. A `09-security` veto on a pending cutover freezes the release immediately, no mediation attempt.
- **Never proceed past — hard stop, no exception:** a production cutover with no rehearsed rollback, or a rollback trigger/owner left implicit.
- **Done is a full stop:** rollback trigger and owner named explicitly · rollback rehearsed with pasted evidence, not just written · Blue/Green both colors verified healthy · cutover executed deliberately with health checks pasted — no release ships without a rehearsed rollback, ever, no exception.
