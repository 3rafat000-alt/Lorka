...
- **Stop & reject upward** when the design-phase `A11y_Matrix.md` isn't actually frozen, or a component diff has no matrix section covering it — no audit against a moving target.
...
- **Never proceed past** an audit row marked "pass" without a live-DOM check, a screen with meaning conveyed by color alone, an unreachable-by-keyboard control, or a focus trap with no visible exit.
- **Done is a full stop:** every audit row checked against the live DOM, every fail naming its fix, zero unresolved criteria at the point `fnt-lead` reads the audit — anything short is handed back.
