...
| `sofi_scan.py` (mode `design`) | `company/os/toolkit/ceo/sofi_scan.py` | `dsn-lead`, `dsn-brand-designer` (via `/sofi-audit` or directly at Gate 2) | Grep-first static sweep: hardcoded hex/px, `!important`, missing `alt`/`aria`, div-as-button, no-reduced-motion, RTL issues, generic-AI-UI smells (centered hero, three equal cards, one accent color) — the mechanical half of the anti-generic-UI checklist. |
| `sofi_scan.py` (mode `flow`) | `company/os/toolkit/ceo/sofi_scan.py` | `dsn-ux-architect` (rare — mostly useful on a brownfield project with an existing routes/views tree to reconcile against a fresh flow diagram) | Routes → views map, orphan/dead-end view detection — a mechanical cross-check against `dsn-ux-architect`'s own dead-end/recovery-path discipline. |
...
| `feature_scan.py` | `company/os/toolkit/ceo/feature_scan.py` | `dsn-lead` (rare — a Gate-2 re-open needing a fast static sweep of what already exists before re-designing) | 0-token static location pass, shared across rooms. |
| `gemini_bridge.py` + `gemini_review.py` | `company/os/toolkit/ceo/` | `dsn-lead` (oracle desk, Teaching VII, at genuine decision points — e.g. a contested taste-vs-a11y call that survived the room-level mediation and needs a second opinion before `dsn-lead` rules on it) | Sanitize → push → capture → digest-ingest, same oracle desk every room shares. |
