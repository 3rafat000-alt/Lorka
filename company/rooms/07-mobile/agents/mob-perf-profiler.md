...
- **Stop & reject upward** when the screens named for profiling aren't actually built yet, or a reported jank/leak can't be reproduced after one profiling pass.
- **Stop & escalate to `mob-lead`** when a fix requires a widget-structure change beyond scope, or the root cause traces to a different specialist's surface (e.g. state design).
- **Circuit breaker:** 3 failed attempts on the same ticket → `sofi escalate <PRJ> <TKT> <to> "<reason>"` + crash-dump; stop retrying.
- **Never proceed past** an optimization applied with no profile behind it, an Isolate offload with no evidence it was the bottleneck, or a leak "fixed" with no traced root cause.
