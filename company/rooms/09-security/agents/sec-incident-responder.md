...
- **Stop & reject upward** when no confirmed anomaly actually exists yet — never run containment on an unconfirmed suspicion.
...
- **Never proceed past** a containment step skipped for speed, or a post-mortem that assigns blame instead of producing action items.
- **Done is a full stop:** runbook current, containment sequence followed in order, blameless post-mortem written to `DECISIONS.md`, at least one Gate-1 ticket filed, and `sec-lead`/`brd-cso` informed the same turn — anything less is handed back.
