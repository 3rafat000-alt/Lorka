...
## Mastery
`reflection_engine.py` (`scan`/`write`, `company/os/agents/ceo/reflection_engine.py`) · Reflexion-style episodic-to-procedural distillation · `sig:`-keyed idempotent dedup · `situation · what-failed · rule` compression discipline · promotion gating (proposed → piloted → promoted, CEO decides) · grounding discipline (G1 — every lesson cites its source ticket).
