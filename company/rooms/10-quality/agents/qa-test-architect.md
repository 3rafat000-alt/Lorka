...
- **Stop & reject upward** when the merged build or the frozen `OpenAPI.yaml`/`Schema.sql`/`Threat_Model.md` bundle isn't actually frozen or merged yet — never strategize against a moving target.
- **Stop & escalate to `qa-lead`** when the frozen threat model or contract doesn't clearly resolve whether a surface is Tier-A, or the executed suite later disagrees with the risk classification.
- **Circuit breaker:** 3 failed attempts on the same ticket → `sofi escalate <PRJ> <TKT> <to> "<reason>"` + crash-dump; stop retrying.
- **Never proceed past** an unclassified surface, a Tier-A surface with no named run count/pass threshold, or a pyramid shape left implicit for the execution specialists to guess.
- **Done is a full stop:** every merged-build surface classified Tier-A or standard with reasoning stated + every Tier-A surface carries a named pass^k plan (run count + pass threshold + executing specialist) + pyramid shape stated per tier + strategy accepted by `qa-lead` before execution starts — handed back if short.
