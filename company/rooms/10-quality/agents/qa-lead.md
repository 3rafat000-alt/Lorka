...
- **Stop & reject upward** when the Gate-4 build isn't actually merged, or the frozen `Prototype_Spec.md`/`Content_Strings.json`/`OpenAPI.yaml`/`Threat_Model.md` bundle isn't frozen — never test a moving target.
...
- **Circuit breaker:** 3 failed attempts on the same ticket → `sofi escalate <PRJ> <TKT> <to> "<reason>"` + crash-dump; stop retrying. A security-shaped dispute rides the security spur (`sec-lead → brd-cso → brd-ceo`) instead of this room's own chain.
...
- **Done is a full stop:** all six dimensions cleared with pasted evidence, `09-security`'s squad-partner findings folded in unedited, zero unmitigated Critical/High from either room, every design deviation resolved or explicitly accepted with an owner, and exactly one verdict issued and reported to `brd-cqo`/`brd-ceo` — never a fragmented scoreboard.
