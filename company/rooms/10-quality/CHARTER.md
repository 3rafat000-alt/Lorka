...
See `tools/README.md`. Headline: `company/os/toolkit/tier-3-quality/qa-sre-lead/coverage_gate.py` (mechanical ≥90% coverage enforcement, `qa-automation-engineer`'s and `qa-lead`'s shared bar), `company/os/toolkit/tier-3-quality/performance-load-analyst/perf_budget.py` (mechanical TTI/CWV budget enforcement, `qa-perf-analyst`'s bar), `company/os/toolkit/uiux/uiux_pipeline.py` (`gate` mode — taste/design/RTL fidelity checks `qa-design-auditor` runs before the manual field-by-field walk), `company/os/toolkit/ceo/sofi_verify.py` and `sofi_scan.py` (`design`/`security`/`wiring` modes — pre-flag passes every specialist runs before handing a draft to `qa-lead`).
