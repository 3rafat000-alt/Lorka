...
See `tools/README.md`. Headline: `company/os/agents/tier-3-quality/qa-sre-lead/coverage_gate.py` (mechanical ≥90% coverage enforcement, `qa-automation-engineer`'s and `qa-lead`'s shared bar), `company/os/agents/tier-3-quality/performance-load-analyst/perf_budget.py` (mechanical TTI/CWV budget enforcement, `qa-perf-analyst`'s bar), `company/os/agents/uiux/uiux_pipeline.py` (`gate` mode — taste/design/RTL fidelity checks `qa-design-auditor` runs before the manual field-by-field walk), `company/os/agents/ceo/sofi_verify.py` and `sofi_scan.py` (`design`/`security`/`wiring` modes — pre-flag passes every specialist runs before handing a draft to `qa-lead`).
